...
$(() => {
    ...
    var reportGet: JQuery = $("#get-report");
    ...
    if (viewModel.reports.length == 0) {
        ...
        viewModel.reports.forEach((report: Report) => {
            ...
            }).text(report.name).click(() => { embedReport(report); console.log("report", report.name); }));
            reportsList.append(li);
...
var embedOneReport = (report: Report, editMode: boolean = false) => {
    $("#seller-table").hide();
